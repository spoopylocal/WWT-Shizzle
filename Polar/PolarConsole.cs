...
internal static class PolarConsoleApp
{
    ...
    private static readonly string PolarVersion = "1.9.9-test";
    ...
    private static bool RunExternalAutoUpdate()
    {
        try
        {
            ...
            var checkExitCode = RunUpdaterScript(
                "-CheckOnly -CurrentVersion " + QuotePowerShellArg(PolarVersion),
                hidden: true);
            ...
            var args = "-ApplyUpdate -CurrentVersion " + QuotePowerShellArg(PolarVersion) +
                       " -CurrentExe " + QuotePowerShellArg(currentExe) +
                       " -ProcessId " + processId.ToString(CultureInfo.InvariantCulture);
            RunUpdaterScript(args, hidden: true);
